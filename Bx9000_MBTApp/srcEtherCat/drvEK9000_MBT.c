...
EK9000_COUPLER_LIST	ek9000_cplr_list;
SINT32	EK9000_DRV_DEBUG = 1;
SINT32	EK9000_DEV_DEBUG = 0;
...
EK9000_COUPLER * EK9000_Get_Coupler_By_Name( const char * cplrname )
{
	...
	for (	pcoupler=(EK9000_COUPLER *)ellFirst((ELLLIST *)&ek9000_cplr_list);
			pcoupler;	pcoupler = (EK9000_COUPLER *)ellNext((ELLNODE *)pcoupler) )
	{		
...
BUSTERM_SIG_DEF * EK9000_Get_SigDef_By_Cname(char * btcname, char * func, E_EPICS_RTYPE epics_rtype)
{
	UINT32	loop;
...
#ifdef	vxWorks
static void	EK9000_Couplers_Reboot_Hook(int startType)
{
	EK9000_COUPLER  * pcoupler = NULL;
	...
	for(	pcoupler=(EK9000_COUPLER *)ellFirst((ELLLIST *)&ek9000_cplr_list);
			pcoupler != NULL; pcoupler = (EK9000_COUPLER *)ellNext((ELLNODE *)pcoupler) )
	{	/* We don't care about any other resource.
		 * we just want to close socket to EK9000 to make next re-connection easy
		 */
	...
	printf("Close link to all EK9000 couplers!\n");
}
#endif
...
int	EK9000_Coupler_Add( char * cplrname, char * ipaddr, char * init_string )
{
	...
	if(!cplr_list_inited)
	{
		ellInit((ELLLIST *) & ek9000_cplr_list);
#ifdef	vxWorks
		rebootHookAdd((FUNCPTR)EK9000_Couplers_Reboot_Hook);
#endif
		cplr_list_inited = 1;
	}
	...
	{
		errlogPrintf( "EK9000_Coupler_Add Error: Empty coupler name!\n" );
		return	-1;
	}

	if(  ipaddr== NULL || strlen(ipaddr) == 0 || inet_addr(ipaddr) == -1 )
	{
		errlogPrintf( "EK9000_Coupler_Add %s Error: Invalid IP address!\n", cplrname );
		return  -1;
	}

	if( EK9000_Get_Coupler_By_Name(cplrname) )
	{
		errlogPrintf( "EK9000_Coupler_Add %s Error: Already a coupler with this name!\n", cplrname );
		return -1;
	}

	/* Parameters are OK, malloc memory for coupler */
	pcoupler = (EK9000_COUPLER *) malloc ( sizeof(EK9000_COUPLER) );
	if( pcoupler == NULL )
	{
		errlogPrintf(	"EK9000_Coupler_Add %s Error: Failed to malloc %zu bytes for coupler!\n",
						cplrname, sizeof(EK9000_COUPLER) );
		return -1;
	}

	/* Get here, coupler structure malloced, init member */
	memset( (char *)pcoupler, 0, sizeof(EK9000_COUPLER) );
	/* Most of members will be init to 0 except below */
	pcoupler->terminated	= 0; /* EL9011 not installed yet */
	...
	{
		errlogPrintf( "EK9000_Coupler_Add %s Error: Failed to initialize ModBusTCP link!\n", cplrname);
		goto FAILURE;
	}
	...
	{
		errlogPrintf( "EK9000_Coupler_Add %s Error: Failed to create mutex_lock!\n", cplrname);
		goto FAILURE;
	}

	ellInit((ELLLIST *) & (pcoupler->sigptr_list));

	pcoupler->msgQ_id = epicsMessageQueueCreate(OPTHREAD_MSGQ_CAPACITY, sizeof(EK9000_SIGNAL *));
	if(pcoupler->msgQ_id == NULL)
	{
		errlogPrintf(	"EK9000_Coupler_Add %s Error: Failed to create message queue with capacity %u!\n",
	...
	/* for EK9000 with IP 130.199.123.234, we will have a name "EK9123.234" */
	memset(pcoupler->opthread_name, 0, MAX_CA_STRING_SIZE );
	strcpy(pcoupler->opthread_name,"EK9");
	...
	ellAdd( (ELLLIST *)&ek9000_cplr_list, (ELLNODE *)pcoupler);
	return 0;
FAILURE:
...
static int EK9000_Couplers_Init_Once();
static int EK9000_Coupler_Init(EK9000_COUPLER * pcoupler);
static int EK9000_Operation(EK9000_COUPLER * pcoupler);
...
int	EK9000_Terminal_Add( char * cplrname, UINT16 slot, char * btname, char * init_string)
{
	...
	if( cplrname == NULL || strlen(cplrname) == 0 )
	{
		errlogPrintf( "EK9000_Terminal_Add Error: Empty EK9000 coupler name!\n");
		return	-1;
	}

	if(slot == 0 || slot > MAX_NUM_OF_BUSTERM)
	{
		errlogPrintf("EK9000_Terminal_Add Error: Slot number is illegal!\n");
		return -1;
	}

	if(  btname== NULL || strlen(btname) == 0 )
	{
		errlogPrintf( "EK9000_Terminal_Add Error: Empty terminal type name!\n");
		return  -1;
	}

	if ( 0 == strcmp("EK9000", btname) )
	{
		errlogPrintf("EK9000_Terminal_Add Error: EK9000 is a controller name, not a terminal name!\n");
		return  -1;
	}

	/* Parameters are OK, check configuration */
	if( NULL == (pcoupler = EK9000_Get_Coupler_By_Name(cplrname)) )
	{
		errlogPrintf("EK9000_Terminal_Add Error: Can't find EK9000 coupler with name %s!\n", cplrname);
		return -1;
	}

	if( NULL == (pbtdef = EK9000_Get_BTDef_By_Cname(btname)) )
	{
		errlogPrintf("EK9000_Terminal_Add Error: Terminal type %s is not supported yet!\n", btname);
		return -1;
	}
	...
	{	/* EL9011 already installed, no new terminals allowed */
		epicsMutexUnlock(pcoupler->mutex_lock);
		errlogPrintf( "EK9000_Terminal_Add %s Error: EK9000_Terminal_Add called after adding EL9011!\n", cplrname );
		return -1;
	}
	if ( pcoupler->installedBusTerm[slot].pbusterm_img_def != NULL )
	{
		epicsMutexUnlock(pcoupler->mutex_lock);
		errlogPrintf("EK9000_Terminal_Add %s Error: Slot %d is already occupied!\n", cplrname, slot );
		return -1;
	}
	...
	{/* You have to continuously install bus terminals */
		epicsMutexUnlock(pcoupler->mutex_lock);
		errlogPrintf("EK9000_Terminal_Add %s Error: The slot before slot %d is empty!\n", cplrname, slot );
		return -1;
	}
	...
	{/* Reach here, means you just add the terminator */
		...
		for(loop=0; loop < MAX_NUM_OF_BUSTERM+2; loop++)	/* +2 means include coupler 9000 and terminator 9011 */
		{
			if(pcoupler->installedBusTerm[loop].pbusterm_img_def)
			{
				...
				if(pcoupler->installedBusTerm[loop].pbusterm_img_def->busterm_type == BT_TYPE_EL9011)
					break;
			}
		}
...
static int EK9000_Couplers_Init_Once()
{
			...
			pcoupler; pcoupler = (EK9000_COUPLER *)ellNext((ELLNODE *)pcoupler) )
	{
		...
		if(pcoupler->opthread_id == (epicsThreadId)(-1))
		{
			...
			{
				epicsMutexUnlock(pcoupler->mutex_lock);
				errlogPrintf(	"Fatal Error: Failed to create operation thread for EK9000 %s\n",
...
{/* finish me, do we reset EK9000? */
	...
	if ( !pcoupler->terminated )
	{
		...
		printf( "Coupler not terminated!\n"
				"Did you call EK9000_Terminal_Add( %s, <slot>, \"EL9011\", 0 ) in st.cmd?\n",
				MBT_GetName(pcoupler->mbt_link) );
		return -1;
	}

	/* if(EK9000_DRV_DEBUG) */
	printf("Trying to connect to EK9000 %s ...\n", MBT_GetName(pcoupler->mbt_link));

	/*******************************************************************/
	/*********** Set up TCP connection *********************************/
	/*******************************************************************/
	/* try to connect to EK9000 */
	...
	{	/* fail to set up MBT link */
		...
		epicsMutexUnlock(pcoupler->mutex_lock);
		printf("EK9000_Coupler_Init Error: Failed to connect to %s!\n", MBT_GetName(pcoupler->mbt_link));
		return -1;
	}
	/* Successfully connect to EK9000 */
	...
	/********************************************************************************************/
	status = EK9000_MBT_Read_Cplr_ID(pcoupler->mbt_link, pcoupler->couplerID, COUPLER_ID_SIZE*2+2, DFT_MBT_TOUT);
	if(status != 0)
	{
		pcoupler->couplerReady = 0;	/* Not so necessary */
		...
		epicsMutexUnlock(pcoupler->mutex_lock);
		/* if(EK9000_DRV_DEBUG) */
		printf("EK9000_Coupler_Init %s Error: Failed to read coupler ID!\n", MBT_GetName(pcoupler->mbt_link));
		return -1;
	}
	...
	{	/* somehow we can't verify, either fail to read or doesn't match */
		...
		epicsMutexUnlock(pcoupler->mutex_lock);
		if(EK9000_DRV_DEBUG)
			printf(	"EK9000_Coupler_Init %s Error: Failed to validate I/O processing image size!\n"
	...
	/******* Sync outputImage ****************************************************/
	/*****************************************************************************/
	status = EK9000_MBT_Read_Output_Image(	pcoupler->mbt_link, pcoupler->outputImage,
											pcoupler->total_out_words, DFT_MBT_TOUT );
	if(status != 0)
	{/* We fail to sync output image. */
		/* I don't know why the EK9000 keeps saying bus error, hope this would help */
		EK9000_MBT_Reset(pcoupler->mbt_link, DFT_MBT_TOUT);
		pcoupler->couplerReady = 0;	/* Not so necessary */
		/* Force MBT close link, we don't mess up existing err code */
		MBT_Disconnect(pcoupler->mbt_link, 0);
		epicsTimeGetCurrent( &(pcoupler->time_lost_conn) );
		epicsMutexUnlock(pcoupler->mutex_lock);
		if(EK9000_DRV_DEBUG)
			printf(	"EK9000_Coupler_Init %s Error: Failed to sync output processing image to local!\n",
					MBT_GetName(pcoupler->mbt_link));
		return -1;
	}
	/*******************************************************************************************************************************/
	...
	epicsMutexUnlock(pcoupler->mutex_lock);
	/* if(EK9000_DRV_DEBUG) */
	printf("Successfully connected to EK9000 %s!\n", MBT_GetName(pcoupler->mbt_link));
...
static int EK9000_Operation(EK9000_COUPLER * pcoupler)
{
	...
	EK9000_SIGNAL_LIST	sig_wrt_outimg_list, sig_rd_inpimg_list, sig_rd_outimg_list, sig_other_list;
	...
	if(pcoupler == NULL)
	{
		errlogPrintf("EK9000_Operation Error: Invalid coupler ptr!\n" );
		return -1;
	}
	...
	while(TRUE)
	{
		...
		msgQstatus = epicsMessageQueueReceiveWithTimeout(	pcoupler->msgQ_id, &psignal,
															sizeof(EK9000_SIGNAL *), OPTHREAD_MSGQ_TMOUT );
		if(msgQstatus < 0)
		{/* Time out, no request, do heartbeat to avoid link drop */
			if(pcoupler->couplerReady)
			{
				status = EK9000_MBT_TestLink(pcoupler->mbt_link, DFT_MBT_TOUT);
				if(status != 0)
				{
		...
		{/* some requests come in */
			...
			{/* coupler is ready, we should do real operation */
				...
				epicsMutexLock(pcoupler->mutex_lock);
				status = EK9000_MBT_Sync_Both_Image(	pcoupler->mbt_link,		rd_inpimg_wstart,
				...
				{/* successfully synced processing image */
					...
					for (	psignal = (EK9000_SIGNAL *)ellFirst( (ELLLIST *) &sig_rd_inpimg_list );
							psignal; psignal = (EK9000_SIGNAL *)ellNext( (ELLNODE *) psignal ) )
					{
					...
					for (	psignal = (EK9000_SIGNAL *)ellFirst( (ELLLIST *) &sig_wrt_outimg_list );
							psignal; psignal = (EK9000_SIGNAL *)ellNext( (ELLNODE *) psignal ) )
					{
					...
					for (	psignal = (EK9000_SIGNAL *)ellFirst( (ELLLIST *) &sig_rd_outimg_list );
							psignal; psignal = (EK9000_SIGNAL *)ellNext( (ELLNODE *) psignal ) )
					{
						(*(psignal->process_fptr))(psignal->pdevdata, psignal->pextra_arg);
						/* err_code and op_done should be already set by process_fptr, we just callback */
				...
				for (	psignal = (EK9000_SIGNAL *)ellFirst( (ELLLIST *) &sig_other_list );
						psignal; psignal = (EK9000_SIGNAL *)ellNext( (ELLNODE *) psignal ) )
				{
...
/* The memory for EK9000_SIGNAL,EK9000_SIGPTR,EK9000_DEVDATA will be malloced inside */
int	EK9000_Signal_Init(
	dbCommon		*	precord,
	...
	void			*	pextra_arg	)
{
	...
	EK9000_DEVSUPDATA	* pdevsupdata;
	EK9000_SIGPTR		* psigptr;
	EK9000_SIGNAL		* psignal;

	if(precord == NULL)
	{
		errlogPrintf("EK9000_Signal_Init Error: No legal record pointer!\n");
		return -1;
	}

	if(ioString == NULL)
	{
		errlogPrintf("EK9000_Signal_Init %s Error: Invalid ioString pointer!\n", precord->name );
		return -1;
	}
	...
	if (count != 3)
	{
		errlogPrintf("EK9000_Signal_Init %s Error: INP/OUT string %s format is illegal!\n", precord->name, ioString);
		return -1;
	}

	pcoupler = EK9000_Get_Coupler_By_Name(cplrname);
	if(pcoupler == NULL)
	{
		errlogPrintf("EK9000_Signal_Init %s Error: Can't find coupler %s!\n", precord->name, cplrname );
		return -1;
	}

	if(bttype == BT_TYPE_EK9000 && slotnum != 0)
	{
		errlogPrintf("EK9000_Signal_Init %s Error: Coupler %s must be in slot 0!\n", precord->name, cplrname );
		return -1;
	}

	if(bttype != BT_TYPE_EK9000 && (slotnum <= 0 || slotnum > MAX_NUM_OF_BUSTERM) )
	{
		errlogPrintf(	"EK9000_Signal_Init %s Error: Slot %d is invalid. Terminal slots must be <1-%d>!\n",
	...
	{
		errlogPrintf(	"EK9000_Signal_Init %s Error: Coupler %s slot %d is not KL_%d!\n",
	...
	psig_def = EK9000_Get_SigDef_By_Ename(bttype, func, epics_rtype);
	if(psig_def == NULL)
	{
		errlogPrintf(	"EK9000_Signal_Init %s Error: Terminal type %d does not support %s!\n",
	...
	{
		errlogPrintf(	"EK9000_Signal_Init %s Error: Invalid process function ptr!\n",
						precord->name );
		return -1;
	}

	pdevsupdata = (EK9000_DEVSUPDATA *)malloc(sizeof(EK9000_DEVSUPDATA));
	if(pdevsupdata == NULL)
	{
		errlogPrintf(	"EK9000_Signal_Init %s Error: Failed to malloc %zu bytes!\n",
						precord->name, sizeof(EK9000_DEVSUPDATA) );
		return -1;
	}

	memset( (char *)pdevsupdata, 0, sizeof(EK9000_DEVSUPDATA) );
...
/**************************************************************************************************/
static	long    EK9000_MBT_EPICS_Init();
static	long    EK9000_MBT_EPICS_Report(int level);

const struct drvet drvEK9000_MBT = {2,                              /*2 Table Entries */
			     (DRVSUPFUN) EK9000_MBT_EPICS_Report,	/* Driver Report Routine */
			     (DRVSUPFUN) EK9000_MBT_EPICS_Init};	/* Driver Initialization Routine */

epicsExportAddress(drvet,drvEK9000_MBT);

/* implementation */
static	long    EK9000_MBT_EPICS_Init()
{
	return	EK9000_Couplers_Init_Once();
}
...
static	long    EK9000_MBT_EPICS_Report(int level)
{
	...
	printf("\n"EK9000_MBT_DRV_VER_STRING"\n\n");
	if(!cplr_list_inited)
	{
		printf("EK9000_MBT_EPICS_Report Error: Coupler linked list is not initialized!\n\n");
		return 0;
	}
