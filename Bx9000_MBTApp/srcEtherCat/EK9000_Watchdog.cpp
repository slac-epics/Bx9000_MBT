...
{
public:
	EK_Coupler(	const char *	pNameOrIpAddr )
		:	m_fCreated(		FALSE		),
			m_pModBusLink(	NULL		)
	{
		...
		EK9000_COUPLER	*	pCplr	= NULL;
		pCplr	= EK9000_Get_Coupler_By_Name( pNameOrIpAddr );
		if ( pCplr != NULL )
		{
...
extern "C" unsigned int EK_CheckWatchdog( char * pNameOrIpAddr, unsigned int fQuiet )
{
	unsigned short			watchdogElapsed	= FALSE;
	EK_Coupler				bkCoupler(	pNameOrIpAddr ); 
...
extern "C" unsigned int MBT_CheckWatchdog( ModBusTCP_Link mbt_link, unsigned int fQuiet )
{
	...
	MBT_Function3( mbt_link, EK_REG_COUPLER_STATUS, 1, &couplerStatus, timeoutSec );
	if ( couplerStatus & EK_STATUS_WATCHDOG_ELAPSED )
 ...
 */
extern "C" int EK_SetWatchdogType( char * pNameOrIpAddr, unsigned int tyWatchdog )
{
	int						result		= 0;
	EK_Coupler				bkCoupler(	pNameOrIpAddr ); 
	ModBusTCP_Link			mbt_link	= bkCoupler.GetModBusLink();
...
extern "C" int MBT_SetWatchdogType( ModBusTCP_Link mbt_link, unsigned int tyWatchdog )
{
	...
	MBT_Function6(mbt_link, EK_REG_WATCHDOG_TYPE, tyWatchdog, timeoutSec );
...
extern "C" int EK_ResetWatchdog( char * pNameOrIpAddr )
{
	int						result		= 0;
	EK_Coupler				bkCoupler(	pNameOrIpAddr ); 
	ModBusTCP_Link			mbt_link	= bkCoupler.GetModBusLink();
...
extern "C" int MBT_ResetWatchdog( ModBusTCP_Link mbt_link )
{
	...
	MBT_Function6(mbt_link, EK_REG_WATCHDOG_RESET, EK_WATCHDOG_RESET_WORD_0, timeoutSec);
	MBT_Function6(mbt_link, EK_REG_WATCHDOG_RESET, EK_WATCHDOG_RESET_WORD_1, timeoutSec);

	return 0;
}


extern "C" int EK_SetWatchdogTimeout( char * pNameOrIpAddr, unsigned int timeout )
{
	int						result		= 0;
	EK_Coupler				bkCoupler(	pNameOrIpAddr ); 
	ModBusTCP_Link			mbt_link	= bkCoupler.GetModBusLink();
...
extern "C" int MBT_SetWatchdogTimeout( ModBusTCP_Link mbt_link, unsigned int timeout )
{
	...
	result = MBT_Function6(mbt_link, EK_REG_WATCHDOG_TIMEOUT, timeout, timeoutSec);
	if ( result < 0 )
	{
	...
	else
	{
		if ( timeout == 0 )
		{
			...
			printf( "Call EK_SetWatchdogTimeout() to enable.\n\n" );
		}
		else
		{
...
extern "C" int EK_DisableWatchdog( char * pNameOrIpAddr )
{
	return EK_SetWatchdogTimeout( pNameOrIpAddr, 0 );
}
