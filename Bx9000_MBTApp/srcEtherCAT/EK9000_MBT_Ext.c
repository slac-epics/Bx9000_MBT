...
/* It is based on ModBusTCPClnt, but for Beckhoff EK9000 only */
...
int	EK9000_MBT_Verify_Image_Size(
	ModBusTCP_Link mbt_link,
	unsigned short int cal_complex_out_bytes,
	unsigned short int cal_complex_in_bytes,
	...
	unsigned int toutsec)
{
	...
	status = MBT_Function3(mbt_link, COMPLEX_OUT_IMG_BYTES_MREG, 1, &temp, toutsec);
	if(status != 0 || temp != cal_complex_out_bytes)
	{
		if ( EK9000_DRV_DEBUG && status == 0 )
		{
			printf(	"EK9000_MBT_Verify_Image_Size: Controller reports %d bytes of complex output, not %d\n",
					temp, cal_complex_out_bytes );
		}
		retStatus = -1;
	}

	status = MBT_Function3(mbt_link, COMPLEX_IN_IMG_BYTES_MREG, 1, &temp, toutsec);
	if(status != 0 || temp != cal_complex_in_bytes)
	{
		if ( EK9000_DRV_DEBUG && status == 0 )
		{
			printf(	"EK9000_MBT_Verify_Image_Size: Controller reports %d bytes of complex input, not %d\n",
					temp, cal_complex_in_bytes );
		}
		retStatus = -1;
	}
	...
	if(status != 0 || temp != cal_digital_out_bits)
	{
		if ( EK9000_DRV_DEBUG && status == 0 )
		{
			printf(	"EK9000_MBT_Verify_Image_Size: Controller reports %d bits of digital output, not %d\n",
	...
	if(status != 0 || temp != cal_digital_in_bits)
	{
		if ( EK9000_DRV_DEBUG && status == 0 )
		{
			printf(	"EK9000_MBT_Verify_Image_Size: Controller reports %d bits of digital input, not %d\n",
...
int EK9000_MBT_Read_Output_Image(ModBusTCP_Link mbt_link, unsigned short int *pimage, unsigned short int wimg_size, unsigned int toutsec)
{
	...
	char		errString[40];
	/* We don't check if woffset is bigger than 0x800 because we don't inlcude the EK9000_Constant.h */
	/* We don't explicitly check link here because MBT_FunctionX does it */
	/* int MBT_Function3(mbt_link, wRIORoffset, RWordCount, *pRWordData, toutsec); */
...
int EK9000_MBT_Sync_Both_Image(
	ModBusTCP_Link			mbt_link,
