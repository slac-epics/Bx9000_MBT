...
static long init_ai_KL3062(struct aiRecord * pai)
{
	if (pai->inp.type!=INST_IO)
	{
		recGblRecordError(S_db_badField, (void *)pai,
			"devAiKL3062 Init_record, Illegal INP");
		pai->pact=TRUE;
		return (S_db_badField);
	}

	if(Bk9000_Signal_Init((dbCommon *) pai, EPICS_RTYPE_AI, pai->inp.value.instio.string, BT_TYPE_KL3062, Bk9000_Dft_ProcFunc, NULL) != 0)
	{
...
static long read_ai_KL3062(struct aiRecord * pai)
{
...
static long lincvt_ai_KL3062(struct aiRecord	*pai, int after)
{
...
epicsExportAddress(dset, devAiKL3062);
