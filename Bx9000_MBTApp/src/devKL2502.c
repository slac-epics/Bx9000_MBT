...
static long init_ao_KL2502(struct aoRecord * pao)
{
	 ...
	 * Typically, this will be 100.0% - 0.0%
	...
	if ( Bx9000_Signal_Init(	(dbCommon *) pao, EPICS_RTYPE_AO, pao->out.value.instio.string,
...
static long write_ao_KL2502(struct aoRecord * pao)
{
	...
	if (!pao->pact)
	{
		/*	Limit range between 0 and 0x7FFF */
		if( pao->rval < 0 )
			rawDutyCycle = 0;
		else if( pao->rval > 0x7FFF )
			rawDutyCycle = 0x7FFF;
		else
		...
		if ( epicsMessageQueueTrySend(	psignal->pdevdata->pcoupler->msgQ_id,
										(void *)&psignal, sizeof(Bx9000_SIGNAL *) ) == -1 )
		{
